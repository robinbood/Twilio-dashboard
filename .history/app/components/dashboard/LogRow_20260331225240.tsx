...
export const LogRow = React.memo(({ entry, isExpanded, onToggle }: LogRowProps) => {
  ...
  const totalCalls = entry.totalCalls;

  const handleToggle = () => {
    onToggle(entry.phoneNumber);
  };
  ...
  console.log(`[LogRow] Render for ${entry.phoneNumber} completed in ${(endTime - startTime).toFixed(2)}ms`);
}, (prevProps, nextProps) => {
  // Custom comparison function to prevent unnecessary re-renders
  // Only re-render if entry, isExpanded, or onToggle changes
  if (prevProps.entry !== nextProps.entry) {
    console.log(`[LogRow] Re-rendering ${nextProps.entry.phoneNumber}: entry changed`);
    return false;
  }

  if (prevProps.isExpanded !== nextProps.isExpanded) {
    console.log(`[LogRow] Re-rendering ${nextProps.entry.phoneNumber}: isExpanded changed`);
    return false;
  }

  if (prevProps.onToggle !== nextProps.onToggle) {
    console.log(`[LogRow] Re-rendering ${nextProps.entry.phoneNumber}: onToggle changed`);
    return false;
  }

  console.log(`[LogRow] Skipping re-render for ${nextProps.entry.phoneNumber}: props are the same`);
  return true;
});
